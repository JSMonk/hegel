...
Sometimes you can have a different types for the same logic. The most easy example of this case is `identity` function:
...
If you will try to annotate only types with which you currently use this function you will have multiple declaration of the same function:
...
As you can see, you need to define function only for type safity. This functions do not add a new business value to your code. You can try to solve this problem with [Unknown Type](/docs/unknown-type), but you will lose needed types.
```typescript
...
Generic Types is the way to create something like "type function". You can define "type arguments" which can be applied to this "type function" and new type will be returned.
...
// which the same as "{ status: 200, body: { message: "Good response " } }"
const response1 = respondWith({ message: "Good response" });

// Type of response2 is "Response<[1, 2, 3]>"
// which the same as "{ status: 200, body: [1, 2, 3] }"
...
So, there are different syntax in different places.

### Functions

To define generic parameters for function you need to add sequence of needed type variables wrapped in `<` `>` (angle brackets) separated by `,` (comma) before arguments list
...
To define generic parameters for a class you need to add sequence of needed type variables wrapped in `<` `>` (angle brackets) separated by `,` (comma) after class identifier
...
To define generic parameters for a type alias you need to add sequence of needed type variables wrapped in `<` `>` (angle brackets) separated by `,` (comma) after type alias identifier
```typescript
...
As you may understand, noone value will be valid for "type variable". Only arguments which annotated as "type variable" will be an valid value for this type.
...
function length<T>(somethingWithLength: T) {
  // Error: Property "length" are not exists in "T"
