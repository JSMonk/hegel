...
Sometimes you can have a different types for the same logic. The easiest example of this case is `identity` function:
...
As you can see, you need to define function only for type safety. This functions do not add a new business value to your code. You can try to solve this problem with [Unknown Type](/docs/unknown-type), but you will lose needed types.
```typescript
...
To define generic parameters for type alias you need to add sequence of needed type variables wrapped in `<` `>` (angle brackets) separated by `,` (comma) after type alias identifier
```typescript
