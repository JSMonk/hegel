...
As you understand, arguments types will be checked when you will try to apply arguments to a function. But return type will be checked when you implemented function logic. 
...
If you will set `string` type to name and firstly hover on (in [Hegel Playground](/try#GYVwdgxgLglg9mABAQwO4FMDOcC26AUYyeAXIplAE4xgDmiAvIgEQCecIzAlIgN4BQiRJXRQQlJAAMAghmx4ANIgAkvIngC+AOkkBufhqA)) function name and after on argument name, you will see some kind of magic: function is optional outside the function but concrete inside.
...
Sometimes, you don't know how many argument will be applied to the function. JavaScript supports rest arguments. The rest operator '...' group all applied arguments in array of arguments.
...
Sometimes, you need to annotate argument or return type as function. Hegel have syntax for function type annotation `(type1, type2) => returnType`. For example:
...
**Info**: If you will play with previous example you will see a next rule with function type:
You can assign function to another only if arguments wider then declarated and return type is more concret then declarated. This rule sounds like: function is [contravariant](https://en.wikipedia.org/wiki/Covariance_and_contravariance_(computer_science)) by arguments and [covariant](https://en.wikipedia.org/wiki/Covariance_and_contravariance_(computer_science)) by return.

More easy and classical example of this rule:
...
Also, Hegel supports `Function` type which represent any possible function in JavaScript:
