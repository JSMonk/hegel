...
Sometimes (especially after refactoring) you may do refinement which does not do something usefull. As example is provement that type of variable is `number` while variable type is always `number`. In this case Hegel try to notify you about it.
