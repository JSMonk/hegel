...
If you will open this example in [Playground](/try#) then you will see that Hegel doesn't show any type error at the 8 line. But why? 
...
The most simple refinement operator is equality. You only may prove equality of variable and some literal and inside the block this
...
The same logic has switch expression. In each case you prove that variable equals to a value. But with an exception - if you will drop "break", "return" and "throw" statement from case then the next case will include previous prove.
...
The second simple refinement operator is identifier. The main restriction of this refinement that it can be used only insde logical expressions
...
Typeof refinement based on comparasion of return value of `typeof` operator and string literal. You can 
...
If you will use any refinement condition with logical "not" operator or oposite operators like not-equal, strict not-not equal you will prove negative case of your refinement
...
Sometimes (especialy after refactoring) you may do refinement which are not do something usefull. As example is provment that type of variable is `number` while variable type is always `number` In this case Hegel try to notify you about it.
