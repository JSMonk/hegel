...
**Info** If you are familiar with TypeScript or Flow.js, you may know about array types, but both Flow.js and TypeScript provide two ways for array definition: via Array-constructor and via adding `[]` (squere brackets) at the end of the type name:
```typescript
...
**Info** If you are familiar with TypeScript or Flow.js, this "analyzers" will infer `firstElement` as `number` which can create an TypeError in runtime.
...
**Info** The reason of this decision is mutable nature of "length" property of Array, so i can write next code, which will broke my program without this property behaivour.
...
**Info** You may ask "Why do not make length 'readonly' and add refinement for `length` array property?". And the answer is popular way for array clean. In many JavaScript program you may see the next line:
```typescript
someArray.length = 0
```
...
**Info** The reason of this decision is reference nature of JavaScript array. If Hegel will allow this assign you will be able mutate source array via another and get unpredictable problems.
